...
/**
 * Interface para resposta do Context IA
 */
...
/**
 * Envia arquivos e prompt para o Context IA
 * @param files Array de arquivos para enviar
 * @param userPrompt Prompt/mensagem do usuário
 * @param promptId ID do prompt (opcional)
 * @returns Resposta do Context IA
 */
export async function sendToContextIA(
  ...
  if (!apiUrl || !apiUser || !apiPassword) {
    throw new Error('Context IA API credentials not configured');
  }
  ...
  const payloadData = {
    query: userPrompt,
    prompt_id: promptId ? parseInt(promptId, 10) : 0,
  };
  formData.append('data', JSON.stringify(payloadData));
  ...
  if (!response.ok) {
    throw new Error(`Context IA request failed: ${response.statusText}`);
  }
